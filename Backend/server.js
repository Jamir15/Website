...
async function getRoomSensorData(roomName) {
    try {
        ...
        if (frontSnap.exists) {
            ...
            if (typeof frontData.Temperature === "number" && !isNaN(frontData.Temperature)) {
                temperatureValues.push(frontData.Temperature);
            }

            if (typeof frontData.Humidity === "number" && !isNaN(frontData.Humidity)) {
        ...
        if (backSnap.exists) {
            ...
            if (typeof backData.Temperature === "number" && !isNaN(backData.Temperature)) {
                temperatureValues.push(backData.Temperature);
            }

            if (typeof backData.Humidity === "number" && !isNaN(backData.Humidity)) {
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({
                error: "Missing required AI context data."
            });
        }
